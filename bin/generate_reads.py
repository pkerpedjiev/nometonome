...
"""
    ...
    for record in records:
        ...
        start_positions = [
            int(i)
            for i in np.linspace(0, chr_length - args.read_length, num_reads_to_sample)
        ]
        strand = "+"
        ...
        for read_start in start_positions:
            ...
            read_id = (
                record.id.split()[0] + "_" + strand + "_" + str(read_start + 1)
            )  # one-based reads
            ...
            if args.sequencing_error_rate and args.sequencing_error_rate > 0:
                read = "".join(
                    [
                        r
                        if random.random() > args.sequencing_error_rate
                        else random.choice(["A", "C", "G", "U"])
                        for r in read
                    ]
                )
